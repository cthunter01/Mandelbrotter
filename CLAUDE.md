...
- `src/gui/`: `Mandelbrotter_gui` — the wxWidgets layer; headers live beside sources, included as `"gui/x.h"`.
  ...
  `--screenshots DIR` developer mode), and the two files with per-platform code: `window_capture.cpp` and
...
- `src/main.cpp`: the executable. `wxIMPLEMENT_APP_NO_MAIN` must stay in this file (in a static library the
  ...
  settings to the GUI and calls `wxEntry` with no arguments. `src/Mandelbrotter.rc` (Windows only) holds the
  application icon and includes wxWidgets' resources, from which wxMSW loads stock cursors such as
  `wxCURSOR_BULLSEYE`
- `src/icons/`: the application icon (`mandelbrotter.ico` for Windows, `mandelbrotter.png` embedded on Linux),
  generated by `make_icons.sh`
